...
def push_category(schema, name: str, parent_id='2'):
    ...
    category_data['category']['name'] = {'language': [{'attrs': {'id': '1'}, 'value': name}]}
    category_data['category']['description'] = {'language': [{'attrs': {'id': '1'}, 'value': f'Description for {name}'}]}
    category_data['category']['link_rewrite'] = {'language': [{'attrs': {'id': '1'}, 'value': name.lower().replace(" ", "-")}]}
...
def load_categories():
    ...
    # Load the JSON data
    with open("../scraping_results/serialization/categories.json", 'r', encoding='utf-8') as file:
        data = json.load(file)
